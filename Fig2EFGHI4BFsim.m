...
SStemp=(c*rx)*inv(Wbar)*(Wxbar*Wxbar')*inv(Wbar');
TSCovEE=(winsize)*SStemp(1,1);
TSCovEI=(winsize)*SStemp(2,1);
TSCovII=(winsize)*SStemp(2,2);

rT=-inv(Wbar)*Wxbar*rx;

clear eiCurr stm counts edges edgest edgesi I CSD time C CII CRR CXR CXX Cii Cei Cee IeRec II JJ IiRec IxRec VRec J Jx s sx SCee SCei SCii TotalCurr XRCurr;
save Fig2EFGHI4BFsim.mat
